...
# Palette tự động gán màu khi tạo tag mới không chọn màu
TAG_COLORS = [
    "#3b82f6",  # blue
    "#10b981",  # emerald
    "#f59e0b",  # amber
...
def list_tags(
    ...
    counts = dict(
        ...
        .group_by(models.NodeTag.tag_id)
        .all()
    )
...
def update_tag(
    ...
    if body.name is not None:
        conflict = db.query(models.Tag).filter(
            models.Tag.name == body.name, models.Tag.id != tag_id
        ).first()
        if conflict:
...
def set_node_tags(
    node_id: str,
    body: schemas.SetNodeTagsRequest,
    db: Session = Depends(get_db),
    _: models.User = Depends(get_current_user),
):
